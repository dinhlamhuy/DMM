...
const MenuBar: React.FC<Props> = ({ children, onDarkMode, isActive }) => {
  ...
  const storedValue = localStorage.getItem("isOpen") || "true";
  const DefautMode = localStorage.getItem("isDark");
  const [isOpenSlide, setIsOpenSlide] = useState(
    storedValue ? storedValue === "true" : false
  );
    ...
    <>
      <nav
        className={`fixed top-0 ${
          DarkMode
            ? "border-slate-900 bg-black dark:bg-black dark:border-slate-900"
            : "bg-white"
        }   shadow-md z-50 w-full bg-nav border-b border-gray-200 `}
      >
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center justify-start rtl:justify-end">
              ...
              >
                ...
                <span
                  className={`uppercase  projectName self-center text-xl ${
                    DarkMode
                      ? "dark:text-yellow-400 text-yellow-400"
                      : "text-black "
                  }  font-bold sm:text-2xl whitespace-nowrap`}
                >
                  Device Management
                </span>
              </button>
      ...
      <aside
        ...
        className={`fixed shadow-xl ${
          DarkMode
            ? "dark:shadow-gray-900  dark:border-slate-900  dark:bg-black shadow-gray-900 border-slate-900 bg-black"
            : "bg-white"
        } top-0 left-0 z-40 w-60 h-screen pt-20 
        ${
          isOpenSlide
      ...
      >
        <div
          className={`h-full px-3 grid gap-4 content-between  ${
            DarkMode
        ...
        >
            ...
            <li
              ...
              className={`${
                isActive === "list"
                  ...
                  : `${
                      DarkMode
                        ? "  hover:text-gray-900 hover:bg-gray-800  "
                        : " text-gray-900 hover:bg-gray-100"
                    }`
              } cursor-pointer `}
            >
              <button className=" flex items-center p-2 rounded-lg  group">
                <FaListAlt
                  className={`text-2xl ${
                    DarkMode ? "dark:text-white text-white" : "text-gray-900"
                  } `}
                />
                <span
                  className={`  ${
            ...
            >
              ...
              >
                ...
                <span
                  className={`  ${
                    DarkMode ? "dark:text-white text-white" : "text-gray-900"
                  } group-hover:text-gray-900 pr-2 flex-1 ms-3 whitespace-nowrap `}
                >
                  {t("lblChangeLng")}
                </span>
              </button>
          ...
          <ul className="space-y-2 font-medium  ">
            ...
            <li className=" select-none">
              <img
                src={DarkMode ? footerblack : footer}
                alt=""
                className="w-full"
              />
            </li>
            <li className="justify-center flex mb-12">
              ...
              <label
                ...
                className={`labelDarkMode  ${
                  DarkMode ? "text-yellow-400" : ""
                }`}
              >
                <IoSunny
                  className={`sun ${DarkMode ? "text-yellow-400" : ""} `}
                />
                <IoIosMoon
                  className={`moon ${DarkMode ? "text-yellow-400" : ""} `}
                />
              </label>
            </li>
          </ul>
      ...
      <div
        className={`p-4    ${isOpenSlide ? "sm:ml-60" : "  "} ${
          DarkMode ? "dark:bg-slate-800 bg-slate-800" : "bg-gray-200"
        } `}
      >
        <div
          className={`   ${
            DarkMode ? "dark:bg-black bg-black" : "bg-white "
          }  p-4 border    border-gray-200   min-h-screen border-dashed rounded-lg  mt-14 pt-1 pb-12`}
        >
          {children}
        </div>
        <p className={` ${DarkMode ? "text-teal-200" : "text-teal-900"} mt-2`}>
          {" "}
          Copyright 2023 &copy; IT-SW LHG Huii & Shan
        </p>
      </div>
    </>
  );
